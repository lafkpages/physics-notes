...
% Command for adding notes in the margin
\makeatletter
...
\newcommand\textbookpage[1]{\hyperlink{page.\number\numexpr#1+12\relax}{#1}}
\newcommand\mysubsection[2]{\subsection[#1]{#1\protect\columnnote{Textbook page \textbookpage{#2}}}}
...
\chapter{Physics Year 1}
\input{proofs/kinetic-energy-formula.tex}

\chapter{Physics Year 2}
\setcounter{section}{4}
\section{Further Mechanics}
\setcounter{subsection}{1}
\mysubsection{Circular Motion}{14}

\subsubsection{Proof of acceleration formula during Uniform Circular Motion}

\input{proofs/circular-motion-acceleration-formula.tex}
...
\begin{center}
  \input{diagrams/transformer.tex}
\end{center}
...
\begin{center}
  \input{diagrams/proton.tex}
  \input{diagrams/neutron.tex}
\end{center}

\section{Thermodynamics}
\mysubsection{Heat and Temperature}{116}

\setcounter{subsubsection}{3}
\subsubsection{Ideal Gas Behaviour}

\paragraph{Boyle's Law}

In a gas, when \(n\) and \(T\) are kept constant, the pressure \(p\) is inversely proportional to the volume \(V\):
...
\begin{align*}
  &\vec{a_R} = -\omega^2 \vec{x} \\
  &\dv[2]{\vec{x}}{t} = -\omega^2 \vec{x} \\
  &\dv[2]{x}{t} + \omega^2 x = 0
\end{align*}

Try \(\vec x = A \sin(\omega t)\):

\begin{align*}
  &\dv[2]{}{t}[A \sin(\omega t)] + \omega^2 (A \sin(\omega t)) = 0 \\
  &\dv{}{t}[A \omega \cos(\omega t)] + \omega^2 A \sin(\omega t) = 0 \\
  &-A \omega^2 \sin(\omega t) + A \omega^2 \sin(\omega t) = 0 \\
  &\hspace{6.5em}0 = 0
\end{align*}

It in fact works for any sinusoidal function.

\input{plots/shm-a-x.tex}
\input{plots/shm-v-x.tex}

Page \textbookpage{154} checkpoint 3:
...
\begin{align*}
  ...
  l &= \frac{g T^2}{4\pi^2} \\
  l &\approx \frac{g (2.5)^2}{4\pi^2} \\
  l &\approx \SI{1.55}{\metre}
\end{align*}

\section{Astrophysics and Cosmology}
\mysubsection{Gravitational Fields}{168}

Given the following properties of the Earth:

\begin{align}
  m = 5.9 \times 10^{24} \text{ kg} \\
  r = 6.371 \times 10^6 \text{ m}
\end{align}

\noindent\textbf{Exam Practice 11A p.\ \textbookpage{177}, Question 3}

The Orion spacecraft consists of a capsule (mass = \SI{8913}{\kilogram})
and a service module (mass = \SI{12337}{\kilogram}) and is designed
to carry four astronauts (mass \SI{80.0}{\kilogram} each). Calculate the
minimum energy needed to move the fully manned spacecraft
completely outside the influence of the gravitational field of
the Earth.

\begin{itemize}
  \item Mass of the Earth = \SI{6.00e24}{\kilogram}
  \item Radius of the Earth = \SI{6.40e6}{\metre}
  \item \(G\) = \SI{6.67e-11}{\newton\metre\squared\per\kilogram\squared}
\end{itemize}

\begin{enumerate}[label=\textbf{\Alph*}]
  \item \SI{2.12e3}{\joule}
  \item \SI{6.30e6}{\joule}
  \item \SI{1.34e12}{\joule}
  \item \SI{1.36e12}{\joule}
\end{enumerate}

Total mass \(m = 8913 + 12337 + 4 \times 80 = \SI{21570}{\kilogram}\).

\begin{align*}
  E_p &= \frac{GMm}{r} \\
  &= \frac{(6.67 \times 10^{-11})(6.00 \times 10^{24})(21570)}{6.40 \times 10^6} \\
  &= \SI{1.35e12}{\joule}
\end{align*}

Hence the answer is C (closest).

\mysubsection{Space}{178}

\paragraph{Wien's Law}

\begin{center}
  \input{plots/wiens-law.tex}
\end{center}

\begin{equation}
  \lambda_1 T_1 = \lambda_2 T_2 = \lambda_3 T_3 = \lambda_\text{max} T = \SI{2.898e-3}{\metre\kelvin}
\end{equation}

\chapter{Other working out}
\input{qna.tex}

\clearpage
\appendix
\input{list-of-equations.tex}

\input{list-of-definitions.tex}

% https://steeven9.github.io/USI-LaTeX/html/math_graph.html
% https://tex.stackexchange.com/a/253040

\chapter{Physics Year 2 Textbook}
\ifdraft{
